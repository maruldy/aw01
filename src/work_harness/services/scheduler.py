...
class SchedulerService:
    def __init__(self, settings: Settings, bootstrap_service: BootstrapService) -> None:
    ...
    def start(self) -> None:
        if not self._configured:
            self._scheduler.add_job(
                self._bootstrap_service.daily_delta_scan,
                CronTrigger(hour=self._settings.daily_delta_scan_hour, minute=0),
                id="daily_delta_scan",
                replace_existing=True,
            )
            self._scheduler.add_job(
                self._bootstrap_service.weekly_digest,
                CronTrigger(
